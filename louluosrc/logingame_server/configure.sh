...
do
	echo ${elem} | grep -q '[.]' 
	if [ $? -gt 0 ]; then
		echo -e "\t @cd ${elem}; make -f makefile.sub -s MAKEFILE_COMM_PATH=\"${MAKEFILE_COMM_PATH}\" APP_OBJ_PATH=\"${APP_OBJ_PATH}\" APP_DBG=${APP_DBG} CXX=\"${CXX}\" FORM=\"${FORM}\" APP_INCLUDE=\"\${APP_INCLUDE}\"" >>${APP_ROOT}/makefile
	fi	
done

echo -e "\t @cd ${MAKEFILE_BASE_PATH}; make -f makefileobj -s MAKEFILE_COMM_PATH=\"${MAKEFILE_COMM_PATH}\" APP_OBJ_PATH=\"${APP_OBJ_PATH}\" APP_DBG=${APP_DBG} CXX=\"${CXX}\" FORM=\"${FORM}\" APP_TARGET=\"${APP_BIN_PATH}/${APP_NAME}\"" >>${APP_ROOT}/makefile
#########################<<clean>>##############################
