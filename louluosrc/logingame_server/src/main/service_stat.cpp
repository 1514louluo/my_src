...
void * service_stat::service_stat_proc( void *param )
{
	...
	MY_LOGERROR("service_stat::service_stat_proc terminated");
	return (void *)0;
...
void service_stat::service_stat_doit( void )
{
	...
	MY_LOGINFO("service_stat::service_stat_proc start");
	int result;
	STU_SERVICE_STAT stat;
	...
	while (true)
	{
		...
		{
			MY_LOGERROR("service stat, unsupported typeid, id="<<stat.service_id);
			continue;
		}
...
void service_stat::init_stat_service( void )
{
	...
	// 登陆注册协议集
	set_service_info(GAMEBOX_SERVER_BOXSERVER_LOGINGAME_ID, GAMEBOX_SERVER_BOXSERVER_LOGINGAME);
	set_service_info(GAMEBOX_SERVER_BOXSERVER_RAPELOGINGAME_ID, GAMEBOX_SERVER_BOXSERVER_RAPELOGINGAME);
	set_service_info(GAMEBOX_SERVER_BOXSERVER_SPECIALLOGINGAME_ID, GAMEBOX_SERVER_BOXSERVER_SPECIALLOGINGAME);
	...
	if (result == -1)
	{
		MY_LOGERROR("failed to start thread service_stat::service_stat_proc");
		return;
	}
	...
	if (result == -1)
	{
		MY_LOGERROR("failed to start thread service_stat::machine_info_stat");
		return;
	}
...
void service_stat::update_service_stat( void )
{
	...
	for (iter=m_map_service.begin(); iter != m_map_service.end(); iter++)
	{
		...
		STAT_LOG("type="<<iter->first<<" path="<<pinfo->cmd_path<<" 60s_req="<<diff<<" current_total_request="
	...
	STAT_LOG("current_client_connection="<<m_current_conn_count<<" 60s_conn="<<diff_conn<<
		" connection_1s="<<(int)((double)diff_conn/60));

	log.attach_stat_log( "current_connection", SDUtility::format(m_current_conn_count));
	//-------------------------- deal with db rw------------------------------------
	{
	...
	STAT_LOG("60s_db_write_count="<<m_db_write_60s_count<<" average_delay_millsec="<<m_db_write_60s_delay
		<<" 60s_db_read_count="<<m_db_read_60s_count<<" average_delay_millsec="<<m_db_read_60s_delay);
	log.attach_stat_log( "60s_db_write_count", SDUtility::format(m_db_write_60s_count));
	log.attach_stat_log( "average_delay_millsec", SDUtility::format(m_db_write_60s_delay));
	log.attach_stat_log( "60s_db_read_count", SDUtility::format(m_db_read_60s_count));
	log.attach_stat_log( "average_delay_millsec", SDUtility::format(m_db_read_60s_delay));
	//-----------------------deal with request response------------------------------
	{
		...
		while (!m_list_req_resp_stat.empty())
		{
			...
			//STAT_LOG("req_resp_delays="<<req_resp_delays);
		}
	...
	STAT_LOG("60s_request_count="<<m_req_resp_count_60s<<" 60s_req_resp_average_delay_millsec="<<m_req_resp_delay_millsec);
	log.attach_stat_log( "60s_request_count", SDUtility::format(m_req_resp_count_60s));
	log.attach_stat_log( "60s_req_resp_average_delay_millsec", SDUtility::format(m_req_resp_delay_millsec));
	...
	MY_LOGDEBUG("60s memcache read count: "<<m_memcache_read_count_60s<<" read delay: "<<m_memcache_read_delay_60s);
	MY_LOGDEBUG("60s memcache write count :"<<m_memcache_write_count_60s<<" write delay: "<<m_memcache_write_delay_60s);
	...
	log.attach_stat_log("cpu", SDUtility::format(m_cpu));
	log.attach_stat_log( "mem", SDUtility::format(int(m_mem)));
	log.attach_stat_log( "load", SDUtility::format(m_overload));

	log.produce_stat_log();
