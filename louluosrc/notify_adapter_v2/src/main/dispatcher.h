...
public:
	...
	block_queue< boost::shared_ptr<HttpRequestPacket> > m_http_request_queue;
...
private:
	...
	static void *http_request_proc(void *param);
	void http_request_doit(void);

	threadpoll_manager m_thread_pool_http;
	...
	void process_offline(boost::shared_ptr<STU_MSG_INFO> prequest);

	void process_heartbeat(boost::shared_ptr<STU_MSG_INFO> prequest); 

	void http_process_boadcast(boost::shared_ptr<HttpRequestPacket>  request );

	void http_reply_ok(boost::shared_ptr<HttpRequestPacket> request);
