	...
	while (true)
	{
		...
		for (i=0; i<fds; i++)
		{
			...
			if (pepoll_events[i].data.fd == m_listen_fd)
			{
				...
				m_http_callback(enum_sock_new_conn_http, NULL, 0, pstu_epoll_data, m_context);
				m_clientcount++;
			...
			if (pepoll_events[i].data.fd == m_pipe_read)
			{
				...
				boost::shared_ptr<STU_EPOLL_RTN_HTTP> stu_epoll_rtn;
...
int httpserver::send_internal(STU_EPOLL_RTN_HTTP *pstu_epoll_rtn)
{
...
int httpserver::dealwith_recv(int clientfd)
{
	...
	int notify_result = m_http_callback(enum_sock_data_recv_http,pstu_epolldata->str_data_recved.c_str(), 
...
void httpserver::close_socket_internal( int clientfd )
{
	...
	if (m_array_epoll_data[clientfd] != NULL)
	{
		m_http_callback(enum_sock_close_http, NULL, 0, m_array_epoll_data[clientfd], m_context);
...
{
	boost::shared_ptr<STU_EPOLL_RTN_HTTP> pstu_rtn(new STU_EPOLL_RTN_HTTP);
