...
int redis_pool::init_pool(int pool_size,  int hold_timeout, std::string &rhost, uint16_t rport, int timeout, int expire)
{
	...
	for(;i < pool_size; i++)
	{
		...
		if(result)
		{
			SYS_LOG_ERROR("<!!!>failed to connect to redis server");
			return -1;
		}
		m_spset_redis_single.add_queue(pclient);
	}	
	
	SYS_LOG_INFO("<===>redis_pool init success!");
	return 0;
}
...
redis_client *redis_pool::fetch_single(void)
{
	...
	for (int i=0; i<3; i++)
	{
		...
		{
			SYS_LOG_ERROR("fetch redis client from pool delayed,delaytime="<<aftertime-beforetime);
		}
		return pclient;
	}

	SYS_LOG_ERROR("fetch redis client from pool failed, try  3 times already");
	return NULL;
}
