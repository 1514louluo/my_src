...
int zmq_client::init_zmq_client( std::string ip, int port,std::string client_identity, zmq_client_callback func )
{
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to start thread zmq_client::reactor_proc");
		return -1;
	}
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to start thread zmq_client::send_msg_proc");
		return -1;
	}
...
void * zmq_client::reactor_proc( void *param )
{
	...
	pthis->reactor_doit();
	SYS_LOG_ERROR("zmq_client::reactor_proc terminiated");
	return (void *)NULL;
...
{
	SYS_LOG_DEBUG("zmq_client::reactor_doit started");
	int result;
	...
	SYS_LOG_DEBUG("conn="<<str_connection);
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("connect to router failed ,conn str="<<str_connection);
		return;
	}
	...
	SYS_LOG_DEBUG("ipc="<<m_ipc_addr);
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to bind socket to ipc,ipc="<<m_ipc_addr);
		return;
	}
	...
	while (true)
	{
		...
		if (poll_result == -1)
		{
			SYS_LOG_ERROR("failed to zmq_poll in dispatch_transport_doit,errstr="<<ZMQ_ERROR<<" errno="<<zmq_errno());
			continue;
		}
		...
		if (currenttime>updatetime)
		{
			...
			if (diff>=5)
			{
				...
				SYS_LOG_DEBUG("ientity="<<m_client_ientity<<" send heartbeat finished");
			}
		}
		...
		if (items [0].revents & ZMQ_POLLIN)    // router, send package to  client
		{
			...
			{
				SYS_LOG_DEBUG("identity="<<m_client_ientity<<"  recv heartbeat, cmdtype="<<stu.cmd_type);
			}
		...
		if (items [1].revents & ZMQ_POLLIN)  // ipc send package to reactor
		{
			...
			if (result != 1)
			{
				SYS_LOG_ERROR("m_queue_ipc.dequeue_nodelay error");
				continue;
			}
			...
			//SYS_LOG_DEBUG("data="<<stu.data<<" dst="<<stu.destination);
			result = sockwrapper.send_pack(m_psocket_router, &stu);
			if (result == -1)
			{
				SYS_LOG_ERROR("failed to send msg to router");
			}
...
void * zmq_client::send_msg_proc( void *param )
{
	...
	zmq_client *pthis = (zmq_client *)param;
	pthis->send_msg_doit();
	SYS_LOG_ERROR("zmq_client::send_msg_proc terminated");
	return (void *)NULL;
...
{
	SYS_LOG_DEBUG("send_msg_doit thread start");
	int result;
	sleep(1);
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to connect to ipc, addr ="<<m_ipc_addr);
		return;
	}
	...
	while (true)
	{
		...
		//SYS_LOG_DEBUG("recv msg from m_queue_send, data="<<pstu->data<<" dst="<<pstu->destination);
