					...
					,const char *szSocket, unsigned long iClientFlag)
{
	...
	for(;i < pool_size; ++i)
	{
		...
		if (result)
		{
			SYS_LOG_ERROR("<!!!>failed to connect to mysql server");
			return -1;
		}
		m_spset_mysql_single.add_queue(pclient);
	}	
	SYS_LOG_INFO("<===>mysql_pool init success!");
	return 0;
}
CMysql *mysql_pool::fetch_single(void)
{
	...
	for (int i=0; i<3; i++)
	{
		...
		{
			SYS_LOG_ERROR("fetch mysql client from pool delayed,delaytime="<<aftertime-beforetime);
		}
		return pclient;
	}

	SYS_LOG_ERROR("fetch mysql client from pool failed, try  3 times already");
	return NULL;
}
