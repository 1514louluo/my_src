...
{
	SYS_LOG_INFO("ready to start  httpserver");
	int result = 0;
	m_port = port;
	...
	{
		SYS_LOG_ERROR("set socket reuse failed");
		return -1;
	}
	...
	{
		SYS_LOG_ERROR("failed to bind socket fd to ip and port="<<m_port);
		return -1;
	}

	if (listen(m_listen_fd, SOMAXCONN)== -1)
	{
		SYS_LOG_ERROR("failed to listen on port="<<m_port);
		return -1;
	}
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to create epoll pipes");
		return -1;
	}
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to create http server epoll read send thread");
		return -1;
	}

	SYS_LOG_INFO("box httpserver start finished");
...
void * httpserver::epoll_reactor( void *param )
{
	...
	SYS_LOG_INFO("epoll_reator thread terminated");
	return (void *)0;
}
...
int httpserver::non_blocking(int socketfd)
{
	...
	{
		//perror ("fcntl");
		SYS_LOG_ERROR("failed to get socket non blocking");
		return -1;
	}
	...
	if (s == -1)
	{
		SYS_LOG_ERROR("failed to set socket non blocking");
		return -1;
	}
...
void httpserver::epoll_reactor_doit( void )
{
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to add listen fd to epoll fd");
		return;
	}
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to add pipe fd to epoll fd");
		return;
	}
	...
	while (true)
	{
		...
		if (fds == 0)
		{
			SYS_LOG_INFO("epoll wait timeout notify");
			continue;
		}
		...
		if (fds == -1)
		{
			...
			SYS_LOG_ERROR("epoll wait error, errno="<<errno<<" errstr="<<strerror(errno));
			return;
		}
		...
		for (i=0; i<fds; i++)
		{
			...
			if (pepoll_events[i].data.fd == m_listen_fd)
			{
				...
				if (new_client_fd == -1)
				{
					SYS_LOG_ERROR("failed to accept new client from listen fd, errstr="<<strerror(errno));
					continue;
				}
				int noblock_result = non_blocking(new_client_fd);
				if (noblock_result == -1)
				{
					SYS_LOG_ERROR("failed to set new client fd to nonblocking");
					continue;
				}
				// set sendbuffer;
				...
				if (optresult == -1)
				{
					SYS_LOG_ERROR("failed to set send buffer size,errno="<<errno<<" errstr="<<strerror(errno));
					continue;
				}
				...
				if (add_result == -1)
				{
					SYS_LOG_ERROR("failed to add new client fd to epoll fd");
					continue;
				}
			...
			if (pepoll_events[i].data.fd == m_pipe_read)
			{
				...
				if (pipe_result == -1)
				{
					SYS_LOG_ERROR("get cmd from pipe read failed, continue");
					continue;
				}
				...
				if (queue_result != 1)
				{
					SYS_LOG_ERROR("failed to get data from queue");
					continue;
				}
				...
				{
					SYS_LOG_ERROR("socket already closed, drop send or close cmd");
					continue;
				}
				...
				{
					SYS_LOG_ERROR("different clientfd and unify tag, skip close or send");
					continue;
				}
				...
				case PIPE_CMD_CLOSE:
					{
						//SYS_LOG_DEBUG("ready to close socket from pipe cmd");
						close_socket_internal(stu_epoll_rtn->clientfd);
					}
					break;
				default:
...
int httpserver::send_internal(STU_EPOLL_RTN *pstu_epoll_rtn)
{
	...
	{
		SYS_LOG_ERROR("client terminal error, not enough buffer for send data, should close socket");
		return -1;
	}
...
int httpserver::dealwith_recv(int clientfd)
{
	...
	case HTTP_CHECK_DATA_ERROR:
		{
			SYS_LOG_ERROR("http request error");
			return -1;
		}
		
		break;
	default:
		SYS_LOG_ERROR("unknown notify result type");
		return 0;
		break;
	}
...
void httpserver::close_socket_internal( int clientfd )
{
	...
	if (del_result == -1)
	{
		SYS_LOG_ERROR("failed to del client fd from epoll fd");
	}
	close(clientfd);
...
int httpserver::send_data( int fd, uint64_t unify_tag, std::string &str_data )
{
	...
	if (pipe_result == -1)
	{
		SYS_LOG_ERROR("failed to pipe cmd send ");
		return -1;
	}
...
int httpserver::close_client( int fd, uint64_t unify_tag )
{
	...
	if (pipe_result == -1)
	{
		SYS_LOG_ERROR("failed to pipe cmd close ");
		return -1;
	}
