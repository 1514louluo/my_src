...
void * http_server::libevent_http_reactor( void *param )
{
	...
	SYS_LOG_ERROR("http_server::libevent_http_reactor terminated");
	return (void *)0;
...
void * http_server::libevent_http_reactor_doit( void )
{
	...
	if (bind_result != 0) 
	{
		SYS_LOG_ERROR("couldn't bind to port "<<(int)port<<" Exiting");
		return (void *)0;
	}
	...
	//if (httpd == NULL) 
	//{
	//	SYS_LOG_ERROR("Error: Unable to listen on "<<proxy_listen<<" "<<proxy_port);
	//	exit(1);
	//}
...
void http_server::api_proxy_handler(struct evhttp_request *req, void *arg) 
{
	...
	const char *request_uri;
	 ...
	 case EVHTTP_REQ_GET:
		 break;
	 default:
		 SYS_LOG_ERROR("unsupported method cmd_type="<<cmd_type);
		 return;
	 }
...
void http_server::init( void )
{
	...
	if (result == -1)
	{
		SYS_LOG_ERROR("failed to create  http_server::libevent_http_reactor");
		return;
	}
