...
typedef int (*httprecv_callback)(enum_notify_type notify_type,const char *pdata, int length, 
...
private:
	...
	block_queue< boost::shared_ptr<STU_EPOLL_RTN> > m_queue_send;

	void close_socket_internal(int clientfd);

	int send_internal(STU_EPOLL_RTN *pstu_epoll_rtn);
