...
/*
    ...
    if ( pos2 == string::npos )
    {
        LOG4CPLUS_DEBUG(logger, "can't find host ");
        return -1;
    }
    else
    {
      ...
      pos2 = sHttpHead.find(host,pos2);
         if ( pos2 == string::npos )
          {
               LOG4CPLUS_DEBUG(logger, "host error ");
    ...
    if( pos2 == string::npos )
    {
        LOG4CPLUS_DEBUG(logger, "can't find REferer "); 
        return -1;
    }
    else
    {
...
{
	LOG4CPLUS_INFO(logger, "HttpRequestPacket sCmd : " << m_str_cmdpath);
	map<string, string>::iterator iter = mapParam.begin();
	for ( ; iter != mapParam.end(); ++iter)	
	{
		LOG4CPLUS_INFO(logger, "HttpRequestPacket param : " << iter->first << " : " << iter->second);
	}
	for ( iter=mapCookie.begin(); iter != mapCookie.end(); ++iter)	
	{
		LOG4CPLUS_INFO(logger, "HttpRequestPacket cookie : " << iter->first << " : " << iter->second);
	}

	for (iter = mapHead.begin(); iter != mapHead.end(); iter++)
	{
		LOG4CPLUS_INFO(logger, "HttpRequestPacket head : " << iter->first << " : " << iter->second);
	}
...
int HttpRequestPacket::decode_post( string& sHttpHead )
{
	...
	if ( pos1 == string::npos )
	{
			LOG4CPLUS_DEBUG(logger, "can't find /");
			return -1;
	}
	string::size_type pos2 = sRequestLine.find(" ", pos1);//find space
	if ( pos2 == string::npos )
	{
			LOG4CPLUS_DEBUG(logger, "can't find space");
			return -1;
	}
	...
	if (iter == mapHead.end())
	{
		LOG4CPLUS_DEBUG(logger,"failed to find CONTENT-LENGTH from headers");
		return -1;
	}
