...
void dispatcher::adapter_cmd_doit( void )
{
	...
	while (true)
	{
		...
		else
		{
			...
			m_zmq_client_pub.send_data(pstu_msg_info->content, m_zmq_router_app_server_identity);
			continue;
...
void dispatcher::process_auth( boost::shared_ptr<STU_MSG_INFO> prequest )
{
	...
	result = tokenid_server.fetch_tokenid(gameinternalno, tokenid);
	...
	m_tcpserver_man.reply_data(prequest->fd, prequest->tag, rtn_buffer);
	MY_LOGDEBUG("socket auth finshed,gameinternalno="<<gameinternalno<<"  tokenid="<<tokenid);
