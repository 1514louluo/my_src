...
namespace cds { namespace intrusive {
    ...
    /** @ingroup cds_intrusive_map
        ...
        - \p OrderedList - ordered list implementation used as bucket for hash set, for example, MichaelList, LazyList.
            The intrusive ordered list implementation specifies the type \p T stored in the hash-set, the \p RCU type
            used by hash-set, the comparison functor for the type \p T and other features specific for the ordered list.
...
#       endif
        ...
        value_type * extract_( Q const& val, Compare cmp )
        {
            ...
            value_type * pItem = m_List.extract_at( pHead, sv, cmp ) ;
            if ( pItem )
                --m_ItemCounter     ;
            return pItem ;
        }
    ...
    public:
        ...
        */
        ...
        /** \anchor cds_intrusive_SplitListSet_rcu_extract
            ...
            See \ref cds_intrusive_MichaelList_rcu_extract "MichaelList<RCU>::extract" to see how
            to use this function.
        */
        template <typename Q>
        value_type * extract( Q const& val )
        {
            return extract_( val, key_comparator() ) ;
        }

        /// Extracts an item from the set using \p pred for searching
        /**
            The function is an analog of \ref cds_intrusive_SplitListSet_rcu_extract "extract(Q const&)"
            but \p cmp is used for key compare.
            \p Less has the interface like \p std::less.
            \p pred must imply the same element order as the comparator used for building the set.
        */
