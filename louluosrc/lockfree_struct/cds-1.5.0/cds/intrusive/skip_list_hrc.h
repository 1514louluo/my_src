...
namespace cds { namespace intrusive { namespace skip_list {
    ...
    protected:
        ...
        virtual void terminate( cds::gc::hrc::ThreadGC * pGC, bool bConcurrent )
        {
            ...
            if ( bConcurrent ) {
                for (unsigned int i = 0; i < nHeight; ++i ) {
                    marked_ptr pNext = next(i).load(CDS_ATOMIC::memory_order_acquire) ;
                    do {} while ( !next(i).compare_exchange_weak( pNext, marked_ptr(), CDS_ATOMIC::memory_order_release, CDS_ATOMIC::memory_order_relaxed ) ) ;
                }
            }
            else {
                for (unsigned int i = 0; i < nHeight; ++i )
