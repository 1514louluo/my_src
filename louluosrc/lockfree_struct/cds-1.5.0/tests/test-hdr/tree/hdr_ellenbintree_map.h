...
#       endif
        ...
        void test()
        {
            ...
            // extract min/max
            {
                ...
                while ( !m.empty() ) {
                    CPPUNIT_ASSERT( m.extract_min( v ) ) ;
                    CPPUNIT_ASSERT( v.first == i ) ;
                    ++i ;
                }
                CPPUNIT_ASSERT( m.empty() )     ;
                CPPUNIT_ASSERT( check_size( m, 0 )) ;
                ...
                extract_functor2 f ;
                while ( !m.empty() ) {
                    CPPUNIT_ASSERT( m.extract_min( i, cds::ref(f) ) ) ;
                    CPPUNIT_ASSERT( f.nKey == i ) ;
                    ++i ;
                }
                CPPUNIT_ASSERT( m.empty() )     ;
                CPPUNIT_ASSERT( check_size( m, 0 )) ;
                ...
                while ( !m.empty() ) {
                    CPPUNIT_ASSERT( m.extract_max( v ) ) ;
                    CPPUNIT_ASSERT( v.first == i ) ;
                    --i ;
                }
                CPPUNIT_ASSERT( m.empty() )     ;
                ...
                i = (int) c_nItemCount - 1 ;
                while ( !m.empty() ) {
                    CPPUNIT_ASSERT( m.extract_max( i, cds::ref(f) ) ) ;
                    CPPUNIT_ASSERT( f.nKey == i ) ;
                    --i ;
                }
                CPPUNIT_ASSERT( m.empty() )     ;
