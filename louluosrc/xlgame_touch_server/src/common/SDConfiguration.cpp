...
void SDConfiguration::insertItem(const SDItemData& item)
{
	...
	{
		LOG4CPLUS_DEBUG(logger, "insert [" << item.m_key << "," << item.m_value << "]");
...
void SDConfiguration::copyItemData(std::vector<SDItemData> &vecItem)
{
	vecItem.clear();

	vecItem.insert(vecItem.begin(), m_content.begin(), m_content.end());

	LOG4CPLUS_INFO(logger, "copy item data size:" << vecItem.size());
}
