...
int RW_detacher::init_RW_detacher(int inittype)
{
	...
	switch(inittype)
	{
		case RW_DETACHER_ALL:
		{
			...
			if(result)
			{
				MY_LOGERROR("<!!!>failed to init_RW_datacher_for_mysql! ");
				return -1;
			}
			result = init_RW_datacher_for_redis();
			if(result)
			{
				MY_LOGERROR("<!!!>failed to init_RW_datacher_for_redis! ");
				return -1;
			}
			break;
		}
		...
		default:
		{
			MY_LOGERROR("<!!!>unknow type in init_RW_datacher! ");
			return -1;
		}
	}
...
int RW_detacher::do_redis_expire(const string_type & key, unsigned int secs)
{
	int rt;
	redis_client *pclient = get_redis_writer();
	if (NULL == pclient)    return -1;
	rt = pclient->expire(key, secs);
	reback_redis_writer(pclient);
	return rt;
}
...
int RW_detacher::do_redis_incrby(const string_type &key, const string_type &value)
{
	int rt;
	redis_client *pclient = get_redis_writer();
	if (NULL == pclient)    return -1;
	rt = pclient->incrby(key, value);
	reback_redis_writer(pclient);
	return rt;
}

int RW_detacher::do_redis_sadd(const string_type &key, const string_type &value)
{
	int rt;
	redis_client *pclient = get_redis_writer();
	if (NULL == pclient)    return -1;
	rt = pclient->sadd(key, value);
	reback_redis_writer(pclient);
	return rt;
}

int RW_detacher::do_redis_sismember(const string_type &key, const string_type &value)
{
	int rt;
	redis_client *pclient = get_redis_reader();
	if (NULL == pclient)    return -1;
	rt = pclient->sismember(key, value);
	reback_redis_reader(pclient);
	return rt;
}
...
//	string_type mysql_wszSocket=config.getString("mysql_wszSocket", "");
	...
	if(result)
	{
		MY_LOGERROR("<!!!>failed to init mysql_pool for read! ");
		return -1;
	}
	...
	if(result)
	{
		MY_LOGERROR("<!!!>failed to init mysql_pool for write! ");
		return -1;
	}
	return 0;
}
...
int RW_detacher::do_mysql_opt_write(string_type &sql)
{
	...
	{
		MY_LOGERROR("exec sql failed, sql="<<sql<<" error="<<e.GetErrMsg());
		return -1;
	}
...
int RW_detacher::do_mysql_opt_read(string_type &sql,  string_map_vector &reply)
{
	CMysql *pclient = NULL;
	...
	catch(CMysqlException& e)
	{
  	...
  	while(pclient->FetchRow())
		...
		for(; field_index < pclient->GetAffectedCols(); ++field_index)
		{
			...
			elem.insert(make_pair(key, value));
		}
		reply.push_back(elem);
    	}
